...
- Frontend: `LakeProvider` (localStorage `gillie:selectedLakeId` ?? `me.primaryLakeId`) + `useLake()`; `LakeSwitcher` rendered in feed top bar and map overlay. Every content query passes `{ lakeId }`; every creation includes `lakeId` in the body.
