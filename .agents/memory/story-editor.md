...
- **Editor and viewer must share ONE media fit mode (object-cover, full-bleed).** Stickers are stage-relative (0-1 over the container), so mixing contain (editor) with cover (viewer) drifts stickers off their placement. flattenDrawing math must match the fit: cover = Math.max scale, composite the overlay into the visible natural-px crop (vx,vy,vw,vh).
- **Viewer chrome auto-hides (~3s); a tap while hidden must only reveal it, not navigate** — otherwise users skip stories accidentally.
